...
public class DepositItemsCommand implements CommandExecutor {
    private final PwingEco plugin;

    public DepositItemsCommand(PwingEco plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length < 2) {
            player.sendMessage("Usage: /deposititems <amount> <currency>");
            return true;
        }
        int amount;
        try {
        ...
        if (plugin.getEconomyManager().depositItems(player, currency, amount)) {
            player.sendMessage("Items deposited successfully.");
        } else {
            player.sendMessage("Failed to deposit items.");
        }
        return true;
    }
}
